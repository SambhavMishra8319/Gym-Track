...
//     { to: "/templates", icon: FolderOpen, label: "Plans" },
...
import { Link, useLocation, useNavigate } from "react-router-dom";
import { Home, BarChart2, User, FolderOpen, PlusCircle } from "lucide-react";
...
export default function Footer() {
    ...
    return (
      <li key={to} className="relative">
        <Link
          to={to}
          aria-current={active ? "page" : undefined}
          className={`flex h-full flex-col items-center justify-center rounded-2xl p-1.5 transition-all duration-200 active:scale-95 ${
            active
              ? "bg-blue-600 text-white shadow-lg shadow-blue-600/25"
              : "text-slate-600 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
          }`}
        >
          <Icon size={active ? 25 : 23} strokeWidth={active ? 2.8 : 2.2} />

          <span
            className={`mt-1 text-[10px] font-bold leading-none tracking-tight ${
              active
                ? "text-white"
  ...
  return (
    // <nav
    //   className="fixed bottom-0 left-0 z-50 h-[5.25rem] w-full border-t border-slate-200 bg-white/95 shadow-[0_-12px_40px_rgba(15,23,42,0.12)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/95 md:hidden"
    <nav
  className="
...
"
    ...
    >
      <div className="relative h-full">
        <ul className="mx-auto grid h-full max-w-sm grid-cols-5 px-2 pb-2 pt-2">
          {navItems.slice(0, 2).map(renderItem)}

          <li className="relative z-10 flex items-center justify-center">
            <div className="absolute h-20 w-20 rounded-full bg-blue-500/25 blur-2xl" />
            ...
            <button
              ...
              onTouchStart={() => setPressed(true)}
              onTouchEnd={() => setPressed(false)}
              className={`relative flex h-16 w-16 items-center justify-center rounded-3xl border-4 border-white bg-gradient-to-br from-blue-600 via-blue-700 to-violet-600 text-white shadow-2xl shadow-blue-600/35 transition-all duration-200 dark:border-slate-900 ${
                pressed ? "scale-95 rotate-6" : "-translate-y-4"
              }`}
              aria-label="Add new workout"
            >
        ...
        <div className="absolute bottom-1 left-1/2 h-1 w-20 -translate-x-1/2 rounded-full bg-slate-300 dark:bg-white/40" />
      </div>
    </nav>
  );
}